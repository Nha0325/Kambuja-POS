...
function POS() {
  ...
  const { data: customers, isLoading: customersLoading } = useFetchData("customers", 1, 100);
  ...
  const handlePayment = async (e) => {
    ...
    const dueAmount = Math.max(0, currentTotal - paid);
    ...
    try {
      ...
      if (res) {
        ...
        setCarts([]);
        if (printReceipt) {
          window.open(`/sale/invoice/${res._id}`, '_blank');
        }
      }
    } catch (error) {
  ...
  if (productsLoading || customersLoading || categoriesLoading) return <Loading />;
    ...
    <div className="p-4 w-full">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Payment">
        <form onSubmit={handlePayment}>
          <select value={customer} onChange={(e) => setCustomer(e.target.value)} className="select select-bordered w-full mb-3" required>
            <option value="">--Select Customer--</option>
            {customers?.map(el => <option key={el._id} value={el._id}>{el.name}</option>)}
          </select>
