...
function SystemHealth() {
  ...
  const handleBackupNow = async () => {
    ...
    try {
      ...
      if (res.data?.success === false) {
        toast.error(res.data.message || t('backup_failed'))
      ...
      toast.error(error.response?.data?.message || t('failed_to_start_backup'))
      window.dispatchEvent(new Event("refetchNotifications"))
    } finally {
  ...
  const handleDeleteBackup = async (fileName) => {
    const ok = await confirm({
      title: t('delete_backup_title'),
      message: t('delete_backup_desc', { fileName }),
    ...
    try {
      ...
      if (res.data?.success === false) {
        toast.error(res.data.message || t('failed_to_delete_backup'))
      } else {
        toast.success(t('backup_deleted'))
        loadHealthData()
      }
    } catch (error) {
      toast.error(error.response?.data?.message || t('failed_to_delete_backup'))
    } finally {
      closeConfirm()
    }
  }
  ...
  if (isError && !healthData) {
    return (
      <div className="flex flex-col items-center justify-center py-32 px-4 max-w-[1600px] mx-auto text-[#020617] dark:text-[#f8fafc]">
        ...
        <h2 className="text-2xl font-bold mb-2">{t('system_health_error')}</h2>
        <p className="text-[#64748b] dark:text-[#a1a1aa] text-center mb-8">{t('failed_to_load_system_health')}</p>
  ...
  // Derived alerts
  const alerts = []
  if (api.status === 'OFFLINE') alerts.push({ type: 'error', message: t('api_unreachable_alert') })
  if (mongodb.status === 'DISCONNECTED') alerts.push({ type: 'error', message: t('mongodb_disconnected_alert') })
  if (backup.lastBackupStatus === 'FAILED') alerts.push({ type: 'warning', message: t('last_backup_failed_alert') })
  ...
  return (
    <div className="flex flex-col gap-6 text-[#020617] dark:text-[#f8fafc] max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-2">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{t('system_health')}</h1>
          <p className="text-sm text-[#64748b] dark:text-[#a1a1aa] mt-1">{t('monitor_api_database_backup')}</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={loadHealthData} className="h-10 px-4 bg-[#ffffff] dark:bg-[#111113] border border-[#e5e7eb] dark:border-[#27272a] text-[#020617] dark:text-[#f8fafc] text-sm font-semibold rounded-lg hover:bg-[#f8fafc] dark:hover:bg-[#09090b] transition-colors shadow-sm inline-flex items-center justify-center gap-2">
            <FaRotateRight className={isLoading ? "animate-spin" : ""} /> {t('refresh')}
          </button>
          <button 
          ...
          >
            <FaCloudArrowUp /> {isBackingUp ? t('processing') : t('backup_now')}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="bg-[#ffffff] dark:bg-[#111113] border border-[#e5e7eb] dark:border-[#27272a] rounded-xl p-5 flex flex-col justify-between h-[120px] shadow-sm">
          <div className="flex justify-between items-start">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${api.status === 'ONLINE' ? 'bg-[#10b981]/10 text-[#10b981]' : 'bg-[#ef4444]/10 text-[#ef4444]'}`}>
              ...
              {api.status || t('unknown')}
            </span>
          </div>
          <div>
            <h3 className="text-xl font-bold tracking-tight">{t('api_server')}</h3>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-xs font-semibold text-[#64748b] dark:text-[#a1a1aa]">{t('uptime_label')}</span>
              <span className="text-[10px] text-[#94a3b8] dark:text-[#64748b] truncate">{api.uptimeSeconds ? Math.floor(api.uptimeSeconds / 60) + ' ' + t('min') : '---'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#ffffff] dark:bg-[#111113] border border-[#e5e7eb] dark:border-[#27272a] rounded-xl p-5 flex flex-col justify-between h-[120px] shadow-sm">
          <div className="flex justify-between items-start">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${mongodb.status === 'CONNECTED' ? 'bg-[#10b981]/10 text-[#10b981]' : 'bg-[#ef4444]/10 text-[#ef4444]'}`}>
              ...
              {mongodb.status || t('unknown')}
            </span>
          </div>
          <div>
            <h3 className="text-xl font-bold tracking-tight" title={mongodb.databaseName}>{t('mongodb')}</h3>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-xs font-semibold text-[#64748b] dark:text-[#a1a1aa]">{t('ping_label')}</span>
        ...
        <div className="bg-[#ffffff] dark:bg-[#111113] border border-[#e5e7eb] dark:border-[#27272a] rounded-xl p-5 flex flex-col justify-between h-[120px] shadow-sm">
          <div className="flex justify-between items-start">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${backup.lastBackupStatus === 'SUCCESS' ? 'bg-[#10b981]/10 text-[#10b981]' : backup.lastBackupStatus === 'FAILED' ? 'bg-[#ef4444]/10 text-[#ef4444]' : 'bg-[#f8fafc] dark:bg-[#09090b] text-[#64748b] dark:text-[#a1a1aa]'}`}>
              {backup.lastBackupStatus || t('none')}
            </span>
          </div>
          <div>
            <h3 className="text-xl font-bold tracking-tight">{t('last_backup')}</h3>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-xs font-semibold text-[#64748b] dark:text-[#a1a1aa]">{t('date_label')}</span>
              <span className="text-[10px] text-[#94a3b8] dark:text-[#64748b] truncate">{backup.lastBackupAt ? formatDate(backup.lastBackupAt, "MMM DD, HH:mm") : t('never')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#ffffff] dark:bg-[#111113] border border-[#e5e7eb] dark:border-[#27272a] rounded-xl p-5 flex flex-col justify-between h-[120px] shadow-sm">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-xs font-semibold text-[#64748b] dark:text-[#a1a1aa]">{t('platform_storage')}</span>
              <span className="text-[10px] text-[#94a3b8] dark:text-[#64748b] truncate">· {t('combined')}</span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-[#ffffff] dark:bg-[#111113] border border-[#e5e7eb] dark:border-[#27272a] rounded-xl shadow-sm overflow-hidden flex flex-col mt-2">
        <div className="p-5 border-b border-[#e5e7eb] dark:border-[#27272a] flex justify-between items-center bg-[#ffffff] dark:bg-[#111113]">
          <h3 className="text-lg font-bold text-[#020617] dark:text-[#f8fafc]">{t('backup_history')}</h3>
          <p className="text-xs text-[#64748b] dark:text-[#a1a1aa] flex items-center gap-1"><FaClock /> {t('showing_recent_backups')}</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f8fafc] dark:bg-[#09090b] border-b border-[#e5e7eb] dark:border-[#27272a]">
                ...
                <th className="px-5 py-3 text-[11px] font-bold text-[#64748b] dark:text-[#a1a1aa] uppercase tracking-wider">{t('file_name')}</th>
                <th className="px-5 py-3 text-[11px] font-bold text-[#64748b] dark:text-[#a1a1aa] uppercase tracking-wider">{t('size')}</th>
                <th className="px-5 py-3 text-[11px] font-bold text-[#64748b] dark:text-[#a1a1aa] uppercase tracking-wider">{t('status')}</th>
                <th className="px-5 py-3 text-[11px] font-bold text-[#64748b] dark:text-[#a1a1aa] uppercase tracking-wider text-right">{t('actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#e5e7eb] dark:divide-[#27272a]">
              {backups?.length > 0 ? backups.map((item) => (
                ...
                <tr>
                  <td colSpan="5" className="px-5 py-8 text-center text-[#94a3b8] dark:text-[#64748b] text-sm">{t('no_backups_found')}</td>
                </tr>
              )}
            </tbody>
          </table>
