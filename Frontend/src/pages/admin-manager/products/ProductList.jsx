    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#09090b] px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-[#64748b] dark:text-[#a1a1aa] mb-2">
          ...
          <span>/</span>
          <span className="font-medium text-[#06b6d4]">{t('products_monitor')}</span>
        </div>
        <h1 className="text-2xl font-semibold text-[#0f172a] dark:text-[#f8fafc] md:text-3xl">{t('global_products_monitor')}</h1>
        <p className="mt-1 text-sm text-[#64748b] dark:text-[#a1a1aa]">{t('global_products_monitor_desc')}</p>
      </div>
      ...
      <div className="mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-xl border border-[#e5e7eb] dark:border-[#27272a] bg-[#ffffff] dark:bg-[#111113] p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#64748b] dark:text-[#a1a1aa]">{t('total_products')}</p>
        ...
        <div className="rounded-xl border border-[#e5e7eb] dark:border-[#27272a] bg-[#ffffff] dark:bg-[#111113] p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#64748b] dark:text-[#a1a1aa]">{t('active_shops')}</p>
        ...
        <div className="rounded-xl border border-[#e5e7eb] dark:border-[#27272a] bg-[#ffffff] dark:bg-[#111113] p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#64748b] dark:text-[#a1a1aa]">{t('low_stock')}</p>
        ...
        <div className="rounded-xl border border-[#e5e7eb] dark:border-[#27272a] bg-[#ffffff] dark:bg-[#111113] p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#64748b] dark:text-[#a1a1aa]">{t('out_of_stock')}</p>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="h-10 rounded-lg border border-[#e5e7eb] dark:border-[#27272a] bg-[#ffffff] dark:bg-[#111113] px-3 text-sm text-[#0f172a] dark:text-[#f8fafc] outline-none transition-colors focus:border-[#06b6d4]"
        >
          <option value="">{t('all_stock_status')}</option>
      ...
      <div className="overflow-x-auto rounded-xl border border-[#e5e7eb] dark:border-[#27272a] bg-[#ffffff] dark:bg-[#111113]">
        <table className="min-w-full text-left">
          <thead className="border-b border-[#e5e7eb] dark:border-[#27272a] bg-[#f8fafc] dark:bg-[#09090b] text-[11px] font-bold uppercase tracking-[0.08em] text-[#64748b] dark:text-[#a1a1aa]">
            <tr>
              <th className="whitespace-nowrap px-4 py-3">{t('product_name')}</th>
              <th className="whitespace-nowrap px-4 py-3">{t('barcode_sku')}</th>
              <th className="whitespace-nowrap px-4 py-3">{t('shop')}</th>
              <th className="whitespace-nowrap px-4 py-3">{t('owner_admin')}</th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={10} className="p-8 text-center text-sm text-[#64748b]">{t('loading')}</td></tr>
            ) : products.length === 0 ? (
              <tr><td colSpan={10} className="p-8 text-center text-sm text-[#64748b]">{t('no_products_found')}</td></tr>
            ) : (
              products.map(p => (
                <tr key={p._id} className="hover:bg-slate-50 dark:hover:bg-[#18181b] transition-colors border-b border-[#e5e7eb] dark:border-[#27272a] last:border-0">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-[#0f172a] dark:text-[#f8fafc]">
                    <div className="font-medium">{p.shopId?.name || t('unknown')}</div>
                    <div className="text-xs text-[#64748b]">{t('code_label')} {p.shopId?.code || "-"}</div>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-[#0f172a] dark:text-[#f8fafc]">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-right">
                    <Link to={`/admin-manager/products/${p._id}`} className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg bg-[#f8fafc] dark:bg-[#18181b] px-3 text-xs font-semibold text-[#0f172a] dark:text-[#f8fafc] transition-colors hover:bg-[#e2e8f0] dark:hover:bg-[#27272a]">
                      <LuEye className="h-3.5 w-3.5" /> {t('details')}
                    </Link>
                  </td>
                </tr>
              ))
