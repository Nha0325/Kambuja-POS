  ...
  return (
    <div className="min-h-screen bg-[#f8f9ff] p-4 sm:p-6 text-slate-800">
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">{t('receipts')}</h1>
          <p className="text-sm text-slate-500">{t('receipts_desc')}</p>
        </div>
      ...
      <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 bg-white border border-[#d7dced] rounded-xl shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">{t('receipts')}</p>
          <p className="mt-1 text-2xl font-extrabold">{totalReceipts}</p>
        </div>
        <div className="p-4 bg-white border border-[#d7dced] rounded-xl shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">{t('paid')}</p>
          <p className="mt-1 text-2xl font-extrabold text-emerald-600">{totalPaidCount}</p>
        </div>
        <div className="p-4 bg-white border border-[#d7dced] rounded-xl shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">{t('partial_due')}</p>
          <p className="mt-1 text-2xl font-extrabold text-red-600">{totalPartialDueCount}</p>
        </div>
        <div className="p-4 bg-white border border-[#d7dced] rounded-xl shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">{t('total_amount')}</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-[#eff4ff] border-b border-[#d7dced]">
              <tr>
                <th className="px-4 py-3 font-semibold text-slate-700">{t('receipt_invoice')}</th>
                <th className="px-4 py-3 font-semibold text-slate-700">{t('date')}</th>
                <th className="px-4 py-3 font-semibold text-right text-slate-700">{t('total')}</th>
                <th className="px-4 py-3 font-semibold text-right text-slate-700">{t('paid')}</th>
                <th className="px-4 py-3 font-semibold text-right text-slate-700">{t('change')}</th>
                <th className="px-4 py-3 font-semibold text-center text-slate-700">{t('payment_status')}</th>
                <th className="px-4 py-3 font-semibold text-center text-slate-700">{t('actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#d7dced]">
              {isLoading ? (
                <tr>
                  <td colSpan="7" className="px-4 py-8 text-center text-slate-500">{t('loading_receipts')}</td>
                </tr>
              ) : filteredSales.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-4 py-8 text-center text-slate-500">{t('no_receipts_found')}</td>
                </tr>
              ) : (
