...
// Base web path for this project (no trailing slash)
if (!defined('APP_BASE')) {
  define('APP_BASE', '/ftc-pos');
}
...
function get_settings(): array {
  ...
  return $stmt->fetch() ?: ['usd_to_khr' => 4100, 'company_name' => 'FTC POS'];
}
...
function telegram_send_message(string $text): bool {
    ...
    $botToken = '8493915749:AAHB4xwUMsW4dPMPrXIRGbWCSttMLJbGy90';
    $chatId   = '-5076506688'; // your group id
...
function telegram_send_document(string $filePath, string $caption = ''): bool {
    ...
    // --- CONFIG: your bot token and group chat id ---
    $botToken = '8493915749:AAHB4xwUMsW4dPMPrXIRGbWCSttMLJbGy90';
    $chatId   = '-5076506688'; // group id
...
function receipt_settings(): array {
  $s = get_settings();
  return [
    'title'         => $s['receipt_title']     ?? ($s['company_name'] ?? 'FTC POS'),
...
function render_closing_report_html(array $bs, array $settings, bool $autoPrint = false): string {
    $company = $settings['company_name'] ?? 'FTC POS';
